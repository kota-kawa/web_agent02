...
class BrowserSession(BaseModel):
	...
	async def get_browser_state_summary(
		...
		if not handlers or not dom_handler_present:
			if not handlers:
				self.logger.warning(
					'No BrowserStateRequestEvent handlers registered. Attempting to reattach watchdogs before requesting browser state.'
				)
			else:
				self.logger.warning(
					'BrowserStateRequestEvent handlers present but DOMWatchdog handler is missing. Reattaching watchdogs before requesting browser state.'
				)
