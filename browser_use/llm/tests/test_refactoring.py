...
class TestRefactoring:
    ...
    async def test_openai_ainvoke_structured(self, mock_get_client):
        ...
        mock_response = MagicMock()
        mock_response.choices = [MagicMock()]
        mock_response.choices[0].message.content = f'{{"country": "{self.EXPECTED_FRANCE_COUNTRY}", "capital": "{self.EXPECTED_FRANCE_CAPITAL}"}}'
        mock_response.usage = None
        mock_get_client.return_value.chat.completions.create = AsyncMock(return_value=mock_response)
    ...
    async def test_groq_ainvoke_structured_fallback(self, mock_parse_failed):
        ...
        mock_parse_failed.return_value = CapitalResponse(country=self.EXPECTED_FRANCE_COUNTRY, capital=self.EXPECTED_FRANCE_CAPITAL)
    ...
    async def test_google_ainvoke_normal(self, mock_send_request):
        ...
        mock_response.json.return_value = {
            'candidates': [{'content': {'parts': [{'text': self.EXPECTED_GERMANY_CAPITAL}]}}]
        }
    ...
    async def test_google_ainvoke_structured(self, mock_send_request):
        ...
        mock_response = MagicMock()
        mock_response.json.return_value = {
            'candidates': [{'content': {'parts': [{'text': f'{{"country": "{self.EXPECTED_FRANCE_COUNTRY}", "capital": "{self.EXPECTED_FRANCE_CAPITAL}"}}'}]}}]
        }
        mock_send_request.return_value = mock_response
