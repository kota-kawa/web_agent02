...
class ChatGroq(BaseChatModel):
	...
	def _is_vision_model(self) -> bool:
		...
		# We avoid hardcoding a list of models to strictly adhere to "do not change available models"
		# and rely on the model name containing keywords.
		model_name = str(self.model).lower()
		if 'vision' in model_name:
			return True
		if 'llava' in model_name:
			return True
		# Add other heuristics if necessary, but currently verified models (Llama 3.3/3.1, Qwen3)
		# in the list are text-only or do not explicitly state vision support in their ID.
		return False
