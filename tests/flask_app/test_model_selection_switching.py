...
def test_switching_from_groq_resets_openai_base_url(monkeypatch, model_selection):
	...
	groq_applied = model_selection.apply_model_selection(
		override={'provider': 'groq', 'model': 'llama-3.1-8b-instant'}
	)
	...
	openai_applied = model_selection.apply_model_selection(
		override={'provider': 'openai', 'model': 'gpt-4.1'}
	)
...
def test_env_groq_url_is_not_reused(monkeypatch, model_selection):
	...
	applied = model_selection.apply_model_selection(override={'provider': 'openai', 'model': 'gpt-4.1'})
...
def test_explicit_base_url_is_respected(monkeypatch, model_selection):
	...
	applied = model_selection.apply_model_selection(
		override={'provider': 'openai', 'model': 'gpt-4.1', 'base_url': custom_url}
	)
