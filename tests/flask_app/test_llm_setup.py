...
from flask_app.llm_setup import _create_selected_llm, ChatGoogle, ChatAnthropic, ChatGroq, ChatOpenAI
...
def test_create_selected_llm_providers(provider, model, expected_client):
    ...
    with patch('flask_app.llm_setup.apply_model_selection') as mock_apply, \
        ...
        assert isinstance(llm_instance, expected_client), f"Incorrect client for provider '{provider}'. Expected {expected_client.__name__}, got {type(llm_instance).__name__}"
