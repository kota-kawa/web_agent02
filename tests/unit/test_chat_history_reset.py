...
def test_new_task_submission_resets_history(monkeypatch):
    ...
    contents_after_reset = ' '.join(entry['content'] for entry in second_data.get('messages', []))
    assert '最初のタスク' not in contents_after_reset
    assert '別のタスク' in contents_after_reset
    assert 'ブラウザ操作エージェントへようこそ' in contents_after_reset

    assert stub_controller.reset_calls == 1
