...
- `directly_open_url` (default: `True`): If we detect a url in the task, we directly open it. When no url is present, the agent now starts from https://www.yahoo.co.jp before taking its first step.
